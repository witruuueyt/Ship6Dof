...
#pragma warning disable 0219
    ...
    public class Drive : BaseDrive
    {
        ...
        [HideInInspector] public bool PositionOverride = false;
        [HideInInspector] public float PositionOverrideValue = 0;
        ...
        private void SetPosition()
        {
            ...
            if (PositionOverride)
            {
                nextPosition= PositionOverrideValue;
            }
            else
            {
                ...
                if (!_istransportsurface)
                {
                    if (!IsRotation)
                    {
                        ...
                        Vector3 localpos = _localstartpos +
                                           _positiondirection *
                                           ((nextPosition + Offset) / Game4AutomationController.Scale) /
                                           _localscale;
            ...
            {
#if GAME4AUTOMATION_INTERACT
                   Game4AutomationPhysics.SetPosition(this,nextPosition);
#endif
            }
        }
            ...
            if (!IsStopped)
                if (!ResetDrive && (CurrentSpeed != 0) && !_StopDrive)
                {
                    CurrentPosition = CurrentPosition +
                                      CurrentSpeed * Game4AutomationController.SpeedOverride * Time.fixedDeltaTime;
                }
