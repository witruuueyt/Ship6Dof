...
namespace game4automation
{
    ...
    public class SceneMouseNavigation : Game4AutomationBehavior
    {
        ...
        private bool touchzoomstart = false;
        ...
        {
            BlockRotation(selected);
            bool touch = Input.touchCount > 0;

            if ((RotateWithLeftMouseButton || touch) && selected)
            {
                blockleftmouserotation = true;
            }
            else
            {
                blockleftmouserotation = false;
            }
        }
...
// Get the renderer for each child object and combine their bounds
            ...
            if (!MouseInOrthoCamera)
            {
                ...
                if (Game4AutomationController.EnableHotkeys)
                {
                    ...
                    {
                        if (selectionmanagernotnull && Game4AutomationController.HotKeyFrontView ==
                            Game4AutomationController.HoteKeyFocfus)
                            if (selectionmanager.SelectedObject == null)
