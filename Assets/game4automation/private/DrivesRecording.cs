...
namespace game4automation
{
    ...
    [CreateAssetMenu(fileName = "DrivesRecording", menuName = "game4automation/Add DrivesRecording", order = 1)]
            ...
            foreach (var recordedDrive in RecordedDrives)
            {
               ...
               // thispath = thispath.Remove(0, 1);
                ...
                if (go != null)
                {
                    ...
                    drive.PositionOverride = true;
                }
                if (drive == null)
                {
                    Debug.LogError(
                        $"game4automation Recording, Drive could not be found [{thispath}] below DrivesRecorder [{handle.DrivesRecorder.name}]");
                }
        ...
        public bool PlayNextFrame(RecordingHandle handle)
        {
            ...
            while (loop && Snapshots[handle.Currentindex].Frame <= handle.Frame)
            {
                ...
                if (!IgnoredDriveIds.Contains(snap.DriveID))
                {
                    ...
                    drive.PositionOverrideValue = snap.Position;
                }
                handle.Currentindex++;
