...
#pragma warning disable 0414
    ...
    public class IKPath : BehaviorInterface
    {
        ...
        [Header("Status")] [game4automation.ReadOnly]
        public bool PathIsActive = false; //!< Is the path active?
        [game4automation.ReadOnly] public bool PathIsFinished = false; //!< Is the path finished?
        [game4automation.ReadOnly] public bool LinearPathActive = false; //!< Is the linear path active?
        [game4automation.ReadOnly] public float LinearPathPos = 0; //!< Position of the linear path
        [game4automation.ReadOnly] public IKTarget CurrentTarget; //!<  Current target of the path
        [game4automation.ReadOnly] public IKTarget LastTarget; //!< Last target of the path
        [game4automation.ReadOnly] public int NumTarget; //!< Number of the current target
        [game4automation.ReadOnly] public bool WaitForSignal; //!< Is the path waiting for a signal?
        [game4automation.ReadOnly] public RobotIK RobotIK; //!<  RobotIK component - assigned robot to this path
        [game4automation.ReadOnly] public Vector3 LastPlannedPosition; //!< Last planned position of the robot
        ...
        private void PositionOnLinearPath()
        {
            ...
            var needslowdowntime = linearpathspeed * SpeedOverride * Game4AutomationController.SpeedOverride /
            ...
            if (!linearacceleration && !lineardeceleration && !slowdownneeded && linearpathspeed <=
                CurrentTarget.LinearSpeedToTarget * SpeedOverride * Game4AutomationController.SpeedOverride)
            {
            ...
            if (!lineardeceleration && linearpathspeed > CurrentTarget.LinearSpeedToTarget * SpeedOverride *
                Game4AutomationController.SpeedOverride * Game4AutomationController.SpeedOverride)
            {
                linearpathacceleration = 0;
